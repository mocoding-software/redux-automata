...
import { automataMiddleware, automataReducer, AutomataState, TaskAutomata, TaskState } from "../../src";
...
describe("Task Automata", () => {
  const promise = new Promise<Data>((accept) => setTimeout(() => accept({ message: "data" }), 200));
  const task: () => Promise<Data> = () => promise;
